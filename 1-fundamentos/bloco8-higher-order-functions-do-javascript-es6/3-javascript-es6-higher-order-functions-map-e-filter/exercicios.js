...
function nameAndAge() {
  const nameAge = books.map((book) => ({author: book.author.name, age: book.releaseYear - book.author.birthYear}));
  nameAge.sort((a, b) => a.age > b.age ? 1 : -1);
...
function oldBooksOrdered() {
  ...
  ageBook.sort((a, b) => a.releaseYear > b.releaseYear ? 1 : -1);
...
function authorWith3DotsOnName() {
  const name = books.filter((book) => book.author.name[1] === '.' && book.author.name[4] === '.' && book.author.name[7] === '.').map((book) => book.name);
